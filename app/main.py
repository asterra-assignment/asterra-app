...
def process_s3_event(event):
    ...
    logger.info("Started processing S3 event")
    try:
        content = response['Body'].read().decode('utf-8')
        geojson_obj = geojson.loads(content)
        if hasattr(geojson_obj, 'is_valid') and geojson_obj.is_valid:
            logger.info("This is a valid GeoJSON file.")
        else:
            logger.info("This is NOT a valid GeoJSON file.")
            return
    except Exception as e:
        logger.info(f"This is NOT a valid GeoJSON file. Exception: {e}")
        return
